...
//    public UserEntity update(UserEntity userEntity, String username) {
//        final UserEntity user = this.getUser(username);
//
//        if (!authorizeUser(user, getUserName()))
//            throw new UnAuthorizedException("You are not authorized access this account");
...
//    }
    ...
    public HttpResponse register( UserEntity user) {
...
//                .setPassword(this.passwordEncoder.encode(user.getPassword()));
    ...
    public HttpResponse deleteById( String username) {
        final UserEntity user = this.getUser(username);
        userRepository.deleteById(user.getId());
        return new HttpResponse(HttpStatus.OK.value(), HttpStatus.OK.getReasonPhrase(), "Your account is deleted");
    }

    public HttpResponse forgotPassword( Credentials credentials) {
    ...
    public HttpResponse changePassword( Credentials credentials) {
    ...
    public Token checkUniqueness( String username) {
    ...
    public UserEntity getUser( String idOrEmailOrMobile) {
        final String regex = "^[A-Za-z0-9+_.-]+@(.+)$";
        Optional<UserEntity> user = null;
    ...
    private boolean authorizeUser( UserEntity user, final String username) {
